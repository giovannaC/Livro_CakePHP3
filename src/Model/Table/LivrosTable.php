...
class LivrosTable extends Table
{
    ...
    {
        $livros = $this->find()
            ->select(['id', 'isbn', 'titulo', 'descricao']);

        if (empty($options['generos'])) {
            $livros
                ->leftJoinWith('Generos')
                ->where(['Generos.descricao IS' => null]);
        } else {
            $livros
                ->innerJoinWith('Generos')
                ->where(['Generos.descricao IN ' => $options['generos']]);
        }

        return $livros->group(['Livros.id']);
    }
}
